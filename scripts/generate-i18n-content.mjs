...
function buildI18nContent() {
	...
	for (const locale of LOCALES) {
		...
		for (const pageId of requiredPages) {
			...
			if (
				!p.heroImage?.startsWith('/images/blog-image') &&
				!p.heroImage?.startsWith('/images/hero-banner') &&
				!p.heroImage?.startsWith('/images/hack-preview') &&
				!p.heroImage?.startsWith('/images/tarkov') &&
				!p.heroImage?.startsWith('/images/escape-from-tarkov')
			) {
